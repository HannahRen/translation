...
if (__DEV__) {
  ...
  debug(
    '启用生产插件 / Enable plugins for production (OccurenceOrder(排序输出), Dedupe(删除重复数据) & UglifyJS(压缩代码)).'
  )
  webpackConfig.plugins.push(
    ...
    minChunks: function(module, count) {
      ...
      if (resourceName) {
        resourceName = resourceName.substring(
          resourceName.lastIndexOf(path.sep) + 1
        )
      }
      var reg = /^(\w)+.vendor/
...
webpackConfig.module.loaders.push({
  test: /\.less$/,
  exclude: null,
  loaders: ['style', 'css?sourceMap&-minimize&modules&localIdentName=[path][name]---[local]---[hash:base64:5]', 'postcss', 'less?sourceMap']
})
webpackConfig.module.loaders.push({
  test: /\.css$/,
  exclude: null,
